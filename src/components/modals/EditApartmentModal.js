...
export default function EditApartmentModal({ isOpen, onClose, onSuccess, apartmentId }) {
  ...
  const fetchApartmentData = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Convert images from URLs to the format expected by ImageUpload
        const formattedImages = apartment.images?.map((url, index) => ({
          id: `existing-${index}`,
          url: url,
          isLocal: false,
          file: null
        ...
        setFormData({
          ...
          contact: {
            ...
            whatsapp: apartment.contact?.whatsapp || ''
          },
          bankDetails: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || 'Failed to update apartment');
      }
    } catch (error) {
    ...
    return (
      <div className="fixed inset-0 bg-black/30 backdrop-blur-sm bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-8 text-center">
          ...
          <p className="mt-2 text-gray-500">Loading apartment data...</p>
        </div>
      </div>
    );
  }

  // ...existing renderStep function from CreateApartmentModal with same content...
  const renderStep = () => {
    switch (currentStep) {
      case 1:
        return (
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-sm text-blue-700">
                Update the bank account details where payments for this apartment will be received.
              </p>
            </div>
      ...
      <div className="bg-white rounded-lg shadow-xl w-[80vw] h-[80vh] max-w-4xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 flex-shrink-0">
          ...
          {currentStep < steps.length ? (
            ...
            >
              {loading ? (uploadingImages ? 'Uploading Images...' : 'Updating...') : 'Update Apartment'}
            </button>
          )}
        </div>
      </div>
